...
import { HOUR_HEIGHT, layoutDay, type CalendarEvent } from '@/features/calendar/types'
...
function eventsOn(events: CalendarEvent[], day: dayjs.Dayjs): CalendarEvent[] {
  return events.filter((event) => dayjs(event.start).isSame(day, 'day'))
}
...
export function TimeGrid({ days, events, onSelectEvent }: TimeGridProps) {
  ...
  const allDayEvents = days.map((day) => eventsOn(events, day).filter((event) => event.allDay))
  ...
  return (
    <div className="calendar-timegrid">
      ...
      {hasAllDay && (
        <div
          className="calendar-timegrid__allday"
          style={{ '--day-count': days.length } as React.CSSProperties}
        >
          <div className="calendar-timegrid__gutter">{messages.calendar.allDay}</div>
          {days.map((day, index) => (
            <div key={day.toString()} className="calendar-allday-cell">
